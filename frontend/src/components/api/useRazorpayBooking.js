    ...
    async (bookingPayload, onSuccess, onError) => {
      ...
      const rzp = new window.Razorpay({
        ...
        prefill: {
          ...
          email: leadPax?.email || "",
          contact: leadPax?.contactNo || "",
>>>>>>> 6374da3 (WIP: save local changes before rebase)
        },
        handler: async (response) => {
          setIsProcessing(true);
          setProcessingStep(0);
          ...
          const confirmBooking = async (payload) => {
            ...
            console.log("[CONFIRM RESPONSE STATUS]", res.status, res.ok);
          ...
          try {
            ...
            const legs = [booking?.outboundLeg, booking?.inboundLeg].filter(
              Boolean,
            );
